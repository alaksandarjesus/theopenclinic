...
<table class="table-auto w-full purchase-order">
    <thead>
        <tr>
    ...
    <tbody>
        @if(!empty($order->uuid))
        @include('pharmacy.purchases.components.items')
        @endif
    </tbody>
    <tfoot>
        <tr>
            ...
            <td class="border border-gray-200 p-0">
                <input type="text" class="form-control w-32 subtotal text-right bg-white format-numeral-value"
                    value="{{$order->subtotal?$order->formatted->subtotal:'0.00'}}" readonly>
        ...
        <tr>
            ...
            <td class="border border-gray-200 p-0">
                <input type="text" class="form-control w-32 taxtotal text-right bg-white format-numeral-value"
                    value="{{$order->tax?$order->formatted->tax:'0.00'}}" readonly>
        ...
        <tr>
            ...
            <td class="border border-gray-200 p-0">
                <input type="text" class="form-control w-32 discount text-right bg-white format-numeral-value"
                    value="{{$order->discount?$order->formatted->discount:'0.00'}}">
            </td>
        </tr>
        <tr>
            ...
            <td class="border border-gray-200 p-0">
                <input type="text" class="form-control w-32 finaltotal text-right bg-white format-numeral-value"
                    value="{{$order->total?$order->formatted->total:'0.00'}}" readonly>
            </td>
        </tr>
        <tr>
...
<script type="text/html" id="pharmacyPurchaseOrderTableRow">
<tr>
    ...
    <td class="border border-gray-200 p-0 text-center">
        <select class="form-select drug w-full">
            ...
            <option value="{{$drug->uuid}}" data-cost="{{$drug->cost}}" data-tax="{{$drug->tax}}">{{$drug->name}}
                ({{$drug->unit}})</option>
            @endforeach
        </select>
    </td>
    ...
    <td class="border border-gray-200 p-0">
        <input type="text" class="form-control w-32 cost update-row-total text-right format-numeral-value" value="0">
    </td>
    <td class="border border-gray-200 p-0">
